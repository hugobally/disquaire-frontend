...
const Listings = () => {
    ...
    const listings = useMemo(() => applyFilters({listings: data.allListing.nodes, filters}), [filters, data])
...
const ListingsGrid = ({listings, className}) => {
    const itemsToRender = useMemo(() => {
        ...
        // Option B - seed
        const initialIntervals = [3, 4, 3, 5, 4, 3, 4, 5, 3, 3, 5, 5, 5, 5, 4, 5, 5, 5, 3, 3, 5, 3, 3, 3, 4, 5, 3, 5, 5, 5, 3, 4, 5, 3, 3, 3, 5, 3, 3, 5, 5, 3, 3, 3, 3, 5, 3, 4, 3, 5]
...
const ListingsGridItem = ({listing, image}) => {
    ...
    return (<article
      ...
      ${note ? 'relative col-span-2 row-span-2 flex h-full w-full flex-col sm:items-center sm:justify-center' : ''} `}
    >
        ...
        {note && (<div>
          <span
              className="sm:absolute sm:top-0 sm:z-20 sm:h-1/3 sm:w-1/2 sm:rotate-3 sm:transform sm:rounded sm:bg-white sm:p-3 sm:shadow-2xl">
            {note}
          </span>
        </div>)}
    </article>)
